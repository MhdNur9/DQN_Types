...
class DQN_attention_Agent:
    ...
    def _build_model(self):
        state_input = Input(shape=(1, self.state_size))

        # raise TypeError("Only integers are allowed")
        l_out = lm(self.no_neurons_per_layer, return_sequences=True)(state_input)
        print("l_out",type(l_out))
        print(l_out.shape)

        # Multi-Head Attention mechanism
        attention_output = MultiHeadAttention(num_heads=self.num_heads, key_dim=self.no_neurons_per_layer)(l_out, l_out)
        #attention_output = MultiHeadAttention(num_heads=self.num_heads, key_dim=self.no_neurons_per_layer)
        print("attention_output", type(attention_output))
        ...
        print(type(flattened_output))
        print(flattened_output.shape)
        raise TypeError("Only integers are allowed")

        # Fully connected layers
        x = Dense(self.no_neurons_per_layer, activation='relu')(flattened_output)
        ...
        # attention_outputs = []
        # for attention_layer in (attention_layers*3):
